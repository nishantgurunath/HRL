...
parser.add_argument('--env-name', default="FetchReach-v1", metavar='G',
...
if args.model_path is None:
    if is_disc_action:
        ...
        policy_mgr = Policy(state_dim, subgoal_dim, log_std=args.log_std, activation_factor=5)
...
def main_loop():
    ...
    for i_iter in range(args.max_iter_num):
        ...
        t0 = time.time()
        loss_m, loss_w = update_params(batch_mgr, batch_wrk)
        t1 = time.time()
        ...
        if i_iter % args.log_interval == 0:
            print('{}\tT_sample {:.4f}\tT_update {:.4f}\tR_min {:.2f}\tR_max {:.2f}\tR_avg {:.2f}\tM_avg {:.2f}\tW_avg {:.2f}'.format(
                i_iter, log['sample_time'], t1-t0, log['min_reward'], log['max_reward'], log['avg_reward'], log['mgr_reward'], log['wrk_reward']))
            print ('Avg_Reward {:.2f}\tM_loss {:.4f},\tW_loss {:.4f}'.format(avg_reward,avg_loss_m,avg_loss_w))
            avg_reward = 0 
            avg_loss_m = 0
            avg_loss_w = 0
