...
class MetaController(nn.Module):
    def __init__(self, in_features=11, out_features=81):
        """
...
class Controller(nn.Module):
    def __init__(self, in_features=92, out_features=81):
    #def __init__(self, in_features=11, out_features=81):
        """
...
class hDQN():
    ...
    def get_quadrant(self,x,y):
        bin_num = np.sqrt(self.subgoals)
        bin_sizex = self.screen_size[1]/bin_num
        bin_sizey = self.screen_size[0]/bin_num     
        binx = x//bin_sizex
        biny = y//bin_sizey
        
        return int(biny*bin_num + binx)
        
    def get_intrinsic_reward(self, goal, position):
        q = self.get_quadrant(position[0],position[1])
        return 0.0 if goal == q else 0.0
    ...
    def action_dict(self):
        actions = np.linspace(-1,1,np.sqrt(self.num_action))
        a1,a2 = np.meshgrid(actions,actions)
        return {k: v for k,v in enumerate(zip(a1.ravel().tolist(),a2.ravel().tolist()))}
